...
export class MessageService {
  ...
  async subscribeToDeliveryStatus(callback: (message: any) => Promise<void>): Promise<void> {
    const client = await this.connect();
    client.subscribe({ 
  ...
  getQueueNames() {
    return {
      ORDER_CREATED: this.QUEUE_ORDER_CREATED,
      DELIVERY_STATUS: this.QUEUE_DELIVERY_STATUS
    };
  }
}
